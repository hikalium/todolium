...
1. `src/engine.test.ts` に T10〜T12 を追加:
   ...
   - T12: 先頭・末尾へのドロップで -24h / +24h が適用される
...
以下の 3 点について確認させてください。

**Q1: モバイルでの postpone ボタン**
現在 `+1d / +2d / +4d / +8d` の4ボタンがあるが、ドラッグ並び替えが使えるようになると重複感がある。モバイル幅では:
- (a) 4ボタン全部残す（折り返してもよい）
- (b) `+1d / +8d` の2ボタンに絞る
- (c) ボタンは非表示にし、並び替えだけにする

どれが好みですか？

**Q2: 先頭・末尾ドロップの締め切り**
先頭（最も急ぎのタスクの上）にドロップした場合、新しい締め切りを `現在の先頭 deadline - 1日` にする予定です。末尾も同様に `+1日`。この固定 ±1日 で問題ありませんか？ それとも別の値が望ましいですか？

**Q3: `set_deadline` イベント型の追加**
ドラッグ並び替えは `postpone` イベント（`ms = targetDeadline - now`）で実装できますが、将来のデバッグ・ログ解析のために `set_deadline`（`deadline` フィールドに絶対時刻を持つ）という専用の型を追加する方が明示的です。シンプルさを優先して `postpone` の流用でいいですか？ それとも新しいイベント型を追加しますか？
