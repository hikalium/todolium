...
端にドロップした場合:
  先頭（A の上）→  floor((now + d1) / 2)   ← 現在時刻と先頭タスクの中間
  末尾（E の下）→  d5 + 24h                ← 固定 +1日
```
...
1. `src/engine.test.ts` に T10〜T12 を追加:
   ...
   - T12: 先頭ドロップで `floor((now + d_first) / 2)`、末尾ドロップで `d_last + 24h` になる
...
| ドロップ位置 | 新しい deadline |
|---|---|
| 先頭（最上位タスクの上）| `floor((now + d_first) / 2)` — 現在時刻と先頭タスクの中間 |
| 末尾（最下位タスクの下）| `d_last + 24h` — 固定 +1日 |

先頭を「`0` と `d_first` の中間」とすることで、先頭タスクより緊急なタスクを自然に挿入できる。`now` を `0` の代わりに使うことで「今よりは先、先頭タスクよりは急ぎ」という意味を持たせる。
