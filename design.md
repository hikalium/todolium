...
```typescript
type EventType = 'add_todo' | 'edit_task' | 'mark_done' | 'postpone' | 'revert';
...
イベントごとのルールの詳細は後述の「設計判断: タスク内容の編集」セクションのイベント表を参照。
...
タスク名の変更をどう表現するか。検討した 3 案のプロスコンスを以下に示す。

### 案 X: イベントをミュータブルにする（同じ eid のまま内容を上書き）

```
events.jsonl の E1 行を直接書き換える
```

| | |
|---|---|
| Pro | 実装が直感的 |
| Con | JSONL の「追記のみ」原則が崩れ、同期時に「どちらのバージョンが新しいか」を別途管理しなければならない。オフライン競合解決が根本から複雑になる。**採用しない。** |

---

### 案 Y: `supersede` ポインター（新しい task_id を作り旧 task_id を継承）

```
[add_todo,  eid=E1, task_id=T1, task="Buy milk"]
... (T1 への操作) ...
[supersede, eid=E5, task_id=T2, inherits=T1, task="Buy oat milk"]
```

T2 は T1 の履歴・状態（deadline、done_at など）を引き継ぎ、タイトルだけ変わる。

| | |
|---|---|
| Pro | タスクの「同一性」と「内容」を明確に分離できる |
| Con | 状態導出が複雑になる（T1 と T2 のイベントを横断して replay する必要がある）。`mark_done` 等が T1 の eid を参照しているのか T2 の eid を参照しているのかが曖昧になる。タスク間に依存関係を持ち込まない方針とも矛盾する。**採用しない。** |
...
```
[add_todo,   eid=E1, task_id=T1, task="Buy milk",      parent_eid=null]
    ↓ parent_eid=E1
[edit_task,  eid=E2, task_id=T1, task="Buy oat milk",  parent_eid=E1]
    ↓ parent_eid=E2
[mark_done,  eid=E3, task_id=T1,                       parent_eid=E2]
```

タスク名は「そのタスクの勝者チェーンの中で、最後に登場する `add_todo` または `edit_task` の `task` フィールド」として導出する。

| | |
|---|---|
| Pro | 既存のチェーンモデルと完全に一致する。競合解決アルゴリズムの変更ゼロ（同じ親を持つイベントのうち新しい方が勝つ、という規則が `edit_task` にもそのまま適用される）。新しい概念を導入しない。 |
| Con | `task` フィールドが `add_todo` と `edit_task` の 2 種類に現れる（型定義で `task` を optional にすれば対応可能）。 |

#### 競合例

```
[E1: add_todo, task="Buy milk"]
    ↓ E1
[E2: edit_task, task="Buy oat milk", at=200]  ← デバイスA オフライン
    ↓ E1
[E3: mark_done, at=250]                        ← デバイスB オフライン

フォーク: E2(at=200) vs E3(at=250) → E3 が勝ち
結果: タスクは done、名前は "Buy milk"（edit は破棄）
```

`mark_done` が後なら完了を優先、`edit_task` が後なら編集を優先、という直感的な動作になる。
...
| `edit_task` | 直前のeid | `task` | タスク名を変更する |
...
| T8 | edit_task と mark_done が競合 (mark_done の at が新しい) | mark_done が勝ち、edit は破棄される |
| T9 | edit_task と mark_done が競合 (edit_task の at が新しい) | edit が勝ち、タスクは todo のまま新しい名前になる |
